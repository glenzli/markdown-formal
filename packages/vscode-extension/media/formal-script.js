(function() {
  ...
  (function() {
    ...
    function clearScheduledRebuild() {
      if (formalWindow.__markdownFormalRebuildTimer !== void 0) {
    ...
    function readSymbols() {
      ...
      try {
        ...
        return Array.isArray(symbols) ? symbols.map((symbol, index) => ({ ...symbol, index })) : [];
      } catch (err) {
    ...
    function inferCurrentFilePathFromLocation(paths) {
      const haystacks = [window.location.pathname, window.location.href, document.baseURI || ""].map((value) => decodePart(value));
      const normalizedPaths = paths.map(normalizePath).filter(Boolean).sort((a, b) => b.length - a.length);
    ...
    function getCurrentFilePath(labels, pages) {
      ...
      if (!formalElement) {
        ...
        const knownPaths = [
          ...
          ...Object.values(labels).map((label2) => label2.filePath)
        ].filter(Boolean);
    ...
    function normalizeHistory(history) {
      ...
      return history.filter((item) => item && typeof item.scroll === "number" && typeof item.filePath === "string").map((item) => {
        const normalized = {
    ...
    function makeHistoryEntry(currentFilePath) {
      if (!currentFilePath) return void 0;
      return {
    ...
    function basenameWithoutExtension(filePath) {
      ...
      return basename.replace(/^\d+-/, "").replace(/^appendix[-_\s]?[a-z0-9]+[-_\s]?/i, "").replace(/\.md$/i, "").replace(/[-_]+/g, " ");
    }
    ...
    function inferVolumeInfoFromPage(page, config) {
      ...
      return inferVolumeInfo({ filePath: page.filePath }, config);
    }
    function findPrimaryTargetId(filePath, labels, unitKey) {
      ...
      return fallback ? `formal-${fallback[0]}` : void 0;
    }
    ...
    function collectChapters(labels, pages, currentFilePath, config) {
      if (pages.length > 0) {
        const currentBook2 = currentFilePath ? inferBookInfoFromPath(currentFilePath, config) : void 0;
        return pages.filter((page) => ["intro", "summary", "chapter", "appendix"].includes(page.kind)).map((page) => pageToChapterItem(page, labels, config)).filter((item) => !currentBook2 || item.bookKey === currentBook2.key).sort((a, b) => {
      ...
      const currentBook = currentFilePath ? inferBookInfoFromPath(currentFilePath, config) : void 0;
    ...
    function getUnitDisplayTitle(chapter, config) {
      ...
      return chapter.unitKind === "appendix" ? uiText(config, "appendix", { label: chapter.unitLabel }) : uiText(config, "chapter", { number: chapter.unitLabel });
    }
    ...
    function nearestSectionLabel(filePath, line, labels) {
      ...
      return Object.values(labels).filter((label) => label.type === "section" && normalizePath(label.filePath) === normalizedFile && typeof label.startLine === "number" && label.startLine + 1 <= line).sort((a, b) => (b.startLine || 0) - (a.startLine || 0))[0];
    }
    ...
    function normalizeDefinitionQuery(value) {
      return String(value || "").normalize("NFKC").replace(/[`*_~#$@()[\]{}（）【】「」『』《》〈〉:：,，.。;；!?！？]/g, "").replace(/\s+/g, " ").trim().toLowerCase();
    }
    function definitionCandidates(definition) {
      return [definition.title || "", ...definition.aliases || []].map((candidate) => normalizeDefinitionQuery(candidate)).filter(Boolean);
    }
    function definitionScore(definition, query) {
      ...
      return definitionCandidates(definition).reduce((best, title) => {
        ...
        if (title.includes(normalizedQuery)) return Math.min(best, 2);
        return best;
      }, Number.MAX_SAFE_INTEGER);
    }
    function definitionSelectionScore(definition, query) {
      const normalizedQuery = normalizeDefinitionQuery(query);
      if (!normalizedQuery) return Number.MAX_SAFE_INTEGER;
      return definitionCandidates(definition).reduce((best, candidate) => {
        if (candidate === normalizedQuery) return Math.min(best, 0);
        if (candidate.startsWith(normalizedQuery)) return Math.min(best, 1);
        if (candidate.includes(normalizedQuery)) return Math.min(best, 2);
        return best;
      }, Number.MAX_SAFE_INTEGER);
    }
    function searchDefinitions(definitions, query, currentFilePath, config, limit = 12) {
      return definitions.filter((definition) => definitionInLookupScope(definition, currentFilePath, config)).map((definition) => ({ definition, score: definitionScore(definition, query) })).filter((item) => item.score < Number.MAX_SAFE_INTEGER).sort((a, b) => {
    ...
    function searchDefinitionsForSelection(definitions, query, currentFilePath, config, limit = 8) {
      const normalizedQuery = normalizeDefinitionQuery(query);
      if (!normalizedQuery) return [];
      return definitions.filter((definition) => definitionInLookupScope(definition, currentFilePath, config)).map((definition) => ({ definition, score: definitionSelectionScore(definition, query) })).filter((item) => item.score < Number.MAX_SAFE_INTEGER).sort((a, b) => {
        if (a.score !== b.score) return a.score - b.score;
        const titleDelta = (a.definition.title || "").length - (b.definition.title || "").length;
        if (titleDelta !== 0) return titleDelta;
        return `${a.definition.filePath}:${a.definition.line}`.localeCompare(`${b.definition.filePath}:${b.definition.line}`);
    ...
    function lineNumberForElement(element, attr) {
    ...
    function findSourceLineElement(line) {
      ...
      const exact = document.querySelector(`[data-line="${targetLine}"]`);
    ...
    function openSymbolSource(symbol) {
      ...
      const sourceDefinition = readDefinitions().find((definition) => normalizePath(definition.filePath) === targetFilePath && definition.line === symbol.sourceLine);
      if (sourceDefinition) {
    ...
    function getDefinitionTemplate(definition) {
      if (definition.index === void 0) return void 0;
      return Array.from(document.querySelectorAll("#formal-definition-templates template[data-definition-index]")).find((template) => Number(template.getAttribute("data-definition-index")) === definition.index);
    }
    function getSymbolDisplayTemplate(symbol) {
      if (symbol.index === void 0) return void 0;
      return Array.from(document.querySelectorAll("#formal-symbol-templates template[data-symbol-display-index]")).find((template) => Number(template.getAttribute("data-symbol-display-index")) === symbol.index);
    }
    ...
    function appendSymbolDisplay(container, symbol) {
    ...
    function appendSymbolMeaning(container, symbol) {
    ...
    function renderSymbolDetail(container, symbol, config) {
    ...
    function renderSymbolMenu(menu, symbols, config) {
      ...
      const selectSymbol = (symbol) => {
        selected = symbol;
        chipGrid.querySelectorAll(".formal-symbol-chip").forEach((chip) => {
    ...
    function renderDefinitionSearchResults(panel, definitions, query, currentFilePath, config) {
    ...
    function getSelectionRect() {
      const selection = window.getSelection();
      if (!selection || selection.rangeCount === 0 || selection.isCollapsed) return void 0;
      const range = selection.getRangeAt(0);
      const rects = Array.from(range.getClientRects()).filter((rect) => rect.width > 0 && rect.height > 0);
      return rects[rects.length - 1] || void 0;
    }
    function showDefinitionSelectionAction(query, results, rect, config) {
      ...
      const top = rect.bottom + actionRect.height + 12 <= window.innerHeight ? rect.bottom + 6 : Math.max(margin, rect.top - actionRect.height - 6);
    ...
    function closeNavDropdowns(containers, except) {
    ...
    function rebuildNav() {
      ...
      try {
        ...
        if (retryCount < 40 && (tocItems.length === 0 && chapters.length === 0 && definitions.length === 0 && symbols.length === 0 || expectsRenderedAnchors && !hasRenderedAnchors)) {
          retryCount++;
          scheduleRebuild(75);
          return;
        }
    ...
    function handleFormalClick(event) {
      ...
      if (!ref && !page && !toc && !chapter) return;
      if (refreshNavIfStale()) {
        event.preventDefault();
        event.stopPropagation();
      ...
      const pages = readPages();
      const currentFilePath = getCurrentFilePath(labels, pages);
      const rawTarget = ref ? ref.getAttribute("data-formal-target") || ref.getAttribute("data-href") || ref.getAttribute("href") : page ? page.getAttribute("data-href") || page.getAttribute("href") : toc ? toc.getAttribute("href") : chapter?.getAttribute("href");
      if (!rawTarget) return;
      const targetInfo = splitTargetUrl(rawTarget, currentFilePath);
      if (!targetInfo.targetId && !chapter && !page) return;
      event.preventDefault();
      event.stopPropagation();
      event.stopImmediatePropagation();
      ...
      navigateToFile(targetInfo.filePath, {
        targetId: targetInfo.targetId || void 0,
        returnTo,
        history
      });
    }
