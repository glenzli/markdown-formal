...
vasm:
  ...
  intent: "Guide target-project AI agents to merge markdown-formal rules into native writing workflows."
  compile:
    format: integrative
    targetLangs: ["zh-CN"]
---
...
不要从远端自动安装或自动更新 skill；不要让 AI 获取并运行未知脚本。升级时重新核对 checksums，再重复接入步骤。
