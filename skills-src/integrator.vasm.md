...
2. 读取 `editor.md`，再读取已有 `.markdown-formal/config.json`、`agent-guide.md` 与 `reference-map.md`。
...
- **写作与引用**：新编号对象用 `#tmp-*` 声明，正文只用 `@h-...`；已有 ID 从 `reference-map.md` 复制；完成时运行 `finish` / `verify`。
...
```text
编辑前运行 npm run formal -- prepare，并读取目标原文与 .markdown-formal/reference-map.md。
...
编辑后运行 npm run formal -- finish <file-or-dir> 与 npm run formal -- verify，保持 Markdown 和 LaTeX 原样。
```
