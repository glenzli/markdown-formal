...
- `config.json` 支持 `"language": "zh"` 或 `"language": "en"`；`scan.exclude` 排除扫描目录，`preview.ignoreHover` 按完整相对路径、裸文件名或 glob 关闭正文 `@hash` recall hover，`debug.previewLog` 临时写入 `.markdown-formal/preview-debug.log` 用于排查空白预览，`debug.markerTraceIds` 可临时追踪单个 hash 的替换前后 token；旧配置缺少字段时会自动合并默认值。
