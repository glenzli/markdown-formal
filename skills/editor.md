...
3. 是否新增、删除或改写了可查询定义？如果有，同步 `.markdown-formal/definitions.json` 中 `source` 指向这些文件的条目，并确保每个 AI 维护条目都有最新 `content`。
...
6. 是否留下 `@tmp-*`、`#tmp-*`、旧文字编号引用、手写章引用或迁移报告中的 unresolved/ambiguous？如果有，先处理再结束。
7. 需要清理建议时运行 `npm run formal -- audit <file-or-dir>`，读取 `.markdown-formal/audit.md`。它只给 AI 审阅清单，不作为提交门禁。
