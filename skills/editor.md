...
- 重要引用附近保留自然语言语义，例如“由谱半径引理 `@h-...` 可得”，不要把一句话写成只有裸 `@h-...`。
- 定义只按定义名进入预览搜索索引，不主动把正文中的术语全部引用化。
- 只有本文明确约定的特殊 LaTeX 记号才写入 `formal-symbols.json`；不要记录通用数学符号或整条推导公式。
...
`finish` 会先 `finalize` 临时 ID，再运行 `verify`。默认只改传入的文件或目录；如果确实写了跨文件的 `@tmp-*` 引用，再显式加 `--all`。
...
定义 marker 用于生成预览搜索材料，不需要 hash ID。搜索只匹配定义名；正文会在命中后展示，但不作为宽泛搜索文本。除非用户明确要求建立引用，AI 不要把所有术语出现都改成显式引用。
...
只维护“记作/denote/write/令/设/where”等句式明确约定过的特殊记号。普通变量、通用函数、一次性推导公式、整条等式不进入符号表。AI 提取符号时只需要维护源位置、pattern、meaning，参数化匹配和运行时缓存由工具生成。
...
- 数学推导依赖某个命题/引理/定理/推论时，用 `@h-...`，但在源码附近保留标题或角色提示，降低 AI 后续阅读负担。
- 需要自然语言标题时用 `@h-....title`。
...
- 定义和符号查询默认只查当前 book；如果确实存在跨书依赖，在 `.markdown-formal/config.json` 写：
...
2. 运行 `npm run formal -- finish <file-or-dir>`。
3. 对旧文字引用运行 `npm run formal -- migrate-text-refs <file-or-dir>` 做 dry-run。
4. 无歧义后运行 `npm run formal -- migrate-text-refs --apply <file-or-dir>`。
5. 默认会同步其他章节指向本章/本卷的 incoming refs；只有明确要限制在目标文件内时才加 `--target-only`。
